...
public:
        ...
        const T* aPreorder, const T* aInorder, int len)
    {
        ...
        for (i = 0; i < len; i++)
            if (aInorder[i] == dataRoot)
                break;
        const int sizeLeft = i; //左子树元素数量
        const int sizeRight = len - i - 1; //右子树元素数量
        if (sizeLeft != 0)
            root->left = RestoreFromPreorderInorder(aPreorder + 1, aInorder, sizeLeft);
        if (sizeRight != 0)
            root->right = RestoreFromPreorderInorder(aPreorder + sizeLeft + 1, aInorder + sizeLeft + 1, sizeRight);
        return root;
    }
    //功能：打印前序遍历、中序遍历、后序遍历
public:
